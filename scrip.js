...
// ==========================================
// FUNCIÓN PARA APLICAR CONFIGURACIÓN VISUAL DESDE SUPABASE
// ==========================================
async function aplicarConfiguracionWeb() {
    try {
        const { data: config, error } = await _supabase.from('configuracion').select('*').eq('id', 1).single();
        ...
        if (config.logo) {
            const logoImg = document.getElementById('logo-pagina'); 
        ...
        // 3. Cambiar WhatsApp (Flotante y en el Footer)
        if (config.wsp) {
            // A) Botón flotante de WhatsApp (Busca el icono de fontawesome)
            const iconoWsp = document.querySelector('.fa-whatsapp');
            if (iconoWsp) {
                const btnWspFlotante = iconoWsp.closest('a');
                if (btnWspFlotante) {
                    btnWspFlotante.href = `https://api.whatsapp.com/send?phone=${config.wsp.replace('+', '')}&text=Hola, vengo de la tienda web y quiero consultar sobre la ropa.`;
                }
            }

            // B) Texto en el Footer
            const footerWsp = document.getElementById('footer-wsp');
            if (footerWsp) {
                footerWsp.textContent = config.wsp;
                footerWsp.href = `https://api.whatsapp.com/send?phone=${config.wsp.replace('+', '')}`;
            }
        }

        // 4. Cambiar Correo en el Footer
        if (config.email) {
        ...
        console.error("Error al aplicar la configuración web:", error);
    }
}